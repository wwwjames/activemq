...
public class MQTTSocket extends AbstractMQTTSocket implements MQTTCodec.MQTTFrameSink, Session.Listener {
    ...
    public void sendToMQTT(MQTTFrame command) throws IOException {
        ...
        try {
            ...
            session.sendBinary(ByteBuffer.wrap(bytes.getData(), 0, bytes.getLength()), Callback.NOOP);
        } catch (Exception e) {
    ...
    public void onWebSocketBinary(ByteBuffer buffer, Callback callback) {
        ...
        try {
            ...
            receiveCounter += bufferBytes.length;
            codec.parse(new DataByteArrayInputStream(bufferBytes), bufferBytes.length );
        } catch (Exception e) {
